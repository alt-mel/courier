...
const mocks = [
  {
    ...
    result: {
      data: {
        signUp: {
          token:
            'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYxODU0NzE1YjExYjExNzJkZjY2M2RhNyIsImlhdCI6MTYzNjEyNDQzNywiZXhwIjoxNjM4NzE2NDM3fQ.l_DoOqDVYwSycLl4dovEn1ypIM71kYWTzU-uLKT9f3s',
          user: {
...
it('handles valid input submission', async () => {
  ...
  const { getByTestId } = render(
    <MockedProvider mocks={mocks} addTypename={false}>
      <SignUpScreen navigation={{ navigate: pushMock }} />
    </MockedProvider>
  );
