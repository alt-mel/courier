...
const mocks = [ {
  ...
  it('renders the Deliveries Screen', () => {
    const tree = render( <MockedProvider><DeliveriesScreen/></MockedProvider>).toJSON();
    expect(tree).toMatchSnapshot();
  });

  it('renders default elements', async () => {

    const { getByTestId } = render(<MockedProvider mocks={mocks} addTypename={false}><DeliveriesScreen/></MockedProvider> );
